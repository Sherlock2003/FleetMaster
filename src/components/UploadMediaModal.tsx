...
export default function UploadMediaModal({ isOpen, onClose, onSuccess, vehicleId }: UploadMediaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(`/api/vehicles/${vehicleId}/media`, {
        method: 'POST',
        body: formData,
        headers: {},
      });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Photo Type</label>
                <input
                  type="text"
                  required
                  className="glass-input w-full px-4 py-3 rounded-xl outline-none"
                  placeholder="e.g., Front View, Interior, Damage"
                  value={type}
